...
\begin{document}
    ...
    \begin{enumerate}[itemsep=2pt]
        ...
        \item 该进程将持续运行直到完成或阻塞，不会被其他进程抢占
    \end{enumerate}
    ...
    \textbf{时间片设置：}在xv6中，时间片由时钟中断频率决定，默认为10ms。进程在时钟中断时会被强制调用 \texttt{yield()} 让出CPU。
    ...
    \textbf{测试配置：}
    \begin{itemize}
        \item 3 个 CPU 密集型进程（优先级：2, 5, 8）
        \item 3 个 IO 密集型进程（优先级：10, 14, 18）
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{优先级调度}：高优先级进程能快速响应，但可能导致低优先级进程饥饿
        \item \textbf{FCFS}：实现简单，但对短作业不友好，可能导致护航效应
        \item \textbf{轮转调度}：公平性好，适合交互式系统，但上下文切换开销较大
        \item \textbf{静态多级队列}：结合了优先级和轮转的优点，但队列边界的设定需要权衡
    \end{itemize}
